...
public class MediaStream implements BirdsIPCamera.IPCameraCallBack {
    BirdsIPCamera mBirdsIPCamera;
...
//        audioStream = new AudioStream(mEasyIPCamera);
    ...
    public void reStartStream() {
        ...
        stopPush();
        mBirdsIPCamera.unrigisterCallback(this);
        mChannelId = mBirdsIPCamera.registerCallback(this);
...
//                                System.arraycopy(h264, 0, newBuf, 0, newBuf.length);
                    ...
                    if (mSWCodec) {
                        if (r > 0) {
                            if(mChannelState == BirdsIPCamera.ChannelState.EASY_IPCAMERA_STATE_REQUEST_PLAY_STREAM) {
                                mBirdsIPCamera.pushFrame(mChannelId, BirdsIPCamera.FrameFlag.EASY_SDK_VIDEO_FRAME_FLAG, stamp, h264, 0, outLen[0]);
                            }
                        }
                    } else {
                        ...
                        if (bufferIndex >= 0) {
                            ...
                            do {
                                ...
                                if (type == 7 || type == 8) {
                                    ...
                                    if(mChannelState == BirdsIPCamera.ChannelState.EASY_IPCAMERA_STATE_REQUEST_PLAY_STREAM) {
                                        mBirdsIPCamera.pushFrame(mChannelId, BirdsIPCamera.FrameFlag.EASY_SDK_VIDEO_FRAME_FLAG, bufferInfo.presentationTimeUs / 1000, h264, 0, mPpsSps.length + bufferInfo.size);
                                    }
                                } else {
                                    ...
                                    if(mChannelState == BirdsIPCamera.ChannelState.EASY_IPCAMERA_STATE_REQUEST_PLAY_STREAM) {
                                        mBirdsIPCamera.pushFrame(mChannelId, BirdsIPCamera.FrameFlag.EASY_SDK_VIDEO_FRAME_FLAG, bufferInfo.presentationTimeUs / 1000, h264, 0, bufferInfo.size);
                                    }
                                }
    ...
    Camera.PreviewCallback previewCallback = new Camera.PreviewCallback() {
        ...
        public synchronized void onPreviewFrame(byte[] data, Camera camera) {
            ...
            if((BirdsIPCamera.ChannelState.EASY_IPCAMERA_STATE_REQUEST_MEDIA_INFO != mChannelState) &&
                    (BirdsIPCamera.ChannelState.EASY_IPCAMERA_STATE_REQUEST_PLAY_STREAM != mChannelState)){
                camera.addCallbackBuffer(data);
                return;
            }
...
//        audioStream.setChannelId(mChannelId);
        ...
        int result = mBirdsIPCamera.startup(iport, BirdsIPCamera.AuthType.AUTHENTICATION_TYPE_BASIC,"", "", "", 0, mChannelId, id.getBytes(), -1, null);
...
//        audioStream.stop();
        ...
        mBirdsIPCamera.resetChannel(mChannelId);
        int result = mBirdsIPCamera.shutdown();
...
//        Log.d(TAG, "kim onIPCameraCallBack, channelId="+channelId+", mChannelId="+mChannelId+", channelState="+channelState);
        ...
        switch(channelState){
            case BirdsIPCamera.ChannelState.EASY_IPCAMERA_STATE_ERROR:
                Log.d(TAG, "kim EASY_IPCAMERA_STATE_ERROR");
                //Util.showDbgMsg(StatusInfoView.DbgLevel.DBG_LEVEL_WARN, "EASY_IPCAMERA_STATE_ERROR");
                break;
            case BirdsIPCamera.ChannelState.EASY_IPCAMERA_STATE_REQUEST_MEDIA_INFO:
                /* 媒体信息 */
                ...
                buffer.putInt(BirdsIPCamera.VideoCodec.EASY_SDK_VIDEO_CODEC_H264);
                ...
                buffer.put(mMei);
                break;
            case BirdsIPCamera.ChannelState.EASY_IPCAMERA_STATE_REQUEST_PLAY_STREAM:
                //Util.showDbgMsg(StatusInfoView.DbgLevel.DBG_LEVEL_INFO, "EASY_IPCAMERA_STATE_REQUEST_PLAY_STREAM");
                startPush();
//                audioStream.startRecord();

                break;
            case BirdsIPCamera.ChannelState.EASY_IPCAMERA_STATE_REQUEST_STOP_STREAM:
